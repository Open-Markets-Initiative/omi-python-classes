...
class SymbolAvailability:
    ...
    def __init__(self, data: bytes, offset: int, parent) -> None:
        self._name = "Symbol Availability"
        self._error = None
        self._parent = parent
        self.raw = None
        self.value = None
...
class QuoteUpdateFlags:
    __slots__ = ('_name', '_error', '_length', '_parent', 'raw', 'unused_6', 'market_session', 'symbol_availability')

    def __init__(self, data: bytes, offset: int, parent) -> None:
        self._name = "Quote Update Flags"
        self._error = None
        self._length = 1
        self._parent = parent
        self.unused_6 = None
        self.market_session = None
        self.symbol_availability = None
        ...
        try:
            value = int.from_bytes(self.raw, byteorder="little", signed=False)
            self.unused_6 = (value >> 2) & 0x3F
            self.market_session = (value >> 1) & 0x1
            self.symbol_availability = value & 0x1
        except Exception as exception:
            self._error = f"Error: {exception}"
...
class QuoteUpdateMessage:
    __slots__ = ('_name', '_error', '_length', '_parent', 'quote_update_flags', 'timestamp', 'symbol', 'bid_size', 'bid_price', 'ask_price', 'ask_size')

    def __init__(self, data: bytes, offset: int, parent) -> None:
        self._name = "Quote Update Message"
        self._error = None
        self._length = 0
        self._parent = parent
        self.quote_update_flags = None
        self.timestamp = None
        self.symbol = None
        self.bid_size = None
        self.bid_price = None
        ...
        self.quote_update_flags = QuoteUpdateFlags(data, current, self)
        if not self.quote_update_flags.valid:
            self._error = self.quote_update_flags._error
            return

        current += self.quote_update_flags._length
        ...
        self.bid_size = BidSize(data, current, self)
        if not self.bid_size.valid:
            self._error = self.bid_size._error
            return

        current += self.bid_size.length

        self.bid_price = BidPrice(data, current, self)
        if not self.bid_price.valid:
            self._error = self.bid_price._error
            return

        current += self.bid_price.length

        self.ask_price = AskPrice(data, current, self)
        if not self.ask_price.valid:
            self._error = self.ask_price._error
            return

        current += self.ask_price.length

        self.ask_size = AskSize(data, current, self)
        if not self.ask_size.valid:
            self._error = self.ask_size._error
            return

        current += self.ask_size.length
...
def MessageData_factory(data: bytes, offset: int, parent, message_type):
    ...
    match message_type:
        ...
        case "Q":
            return QuoteUpdateMessage(data, offset, parent)
