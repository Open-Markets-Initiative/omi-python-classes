...
class OrderIdReference:
    """Order ID of the referenced order"""
    __slots__ = ('_name', '_error', '_parent', 'raw', 'value')

    def __init__(self, data: bytes, offset: int, parent) -> None:
        self._name = "Order Id Reference"
        self._error = None
        self._parent = parent
        self.raw = None
        self.value = None
...
class RetailLiquidityIndicator:
    """Retail Liquidity Indicator identifier"""
    __slots__ = ('_name', '_error', '_parent', 'raw', 'value')

    def __init__(self, data: bytes, offset: int, parent) -> None:
        self._name = "Retail Liquidity Indicator"
        self._error = None
        self._parent = parent
        self.raw = None
        self.value = None
...
class ClearBookMessage:
    __slots__ = ('_name', '_error', '_length', '_parent', 'reserved_1', 'timestamp', 'symbol')

    def __init__(self, data: bytes, offset: int, parent) -> None:
        self._name = "Clear Book Message"
        self._error = None
        self._length = 0
        self._parent = parent
        ...
        self.reserved_1 = Reserved1(data, current, self)
        if not self.reserved_1.valid:
            self._error = self.reserved_1._error
            return

        current += self.reserved_1.length
...
class OrderExecutedMessage:
    __slots__ = ('_name', '_error', '_length', '_parent', 'sale_condition_flags', 'timestamp', 'symbol', 'order_id_reference', 'size', 'price', 'trade_id')
    ...
    def __init__(self, data: bytes, offset: int, parent) -> None:
        ...
        self.order_id_reference = OrderIdReference(data, current, self)
        if not self.order_id_reference.valid:
            self._error = self.order_id_reference._error
            return

        current += self.order_id_reference.length
...
class OrderDeleteMessage:
    __slots__ = ('_name', '_error', '_length', '_parent', 'reserved_1', 'timestamp', 'symbol', 'order_id_reference')
...
class ModifyFlags:
    __slots__ = ('_name', '_error', '_length', '_parent', 'raw', 'unused_7', 'priority')

    def __init__(self, data: bytes, offset: int, parent) -> None:
        self._name = "Modify Flags"
        self._error = None
        self._length = 1
        self._parent = parent
        self.unused_7 = None
        ...
        try:
            value = int.from_bytes(self.raw, byteorder="little", signed=False)
            self.unused_7 = (value >> 1) & 0x7F
            self.priority = value & 0x1
        except Exception as exception:
            self._error = f"Error: {exception}"
...
class OrderModifyMessage:
    __slots__ = ('_name', '_error', '_length', '_parent', 'modify_flags', 'timestamp', 'symbol', 'order_id_reference', 'size', 'price')

    def __init__(self, data: bytes, offset: int, parent) -> None:
        self._name = "Order Modify Message"
        self._error = None
        self._length = 0
        self._parent = parent
        self.modify_flags = None
        self.timestamp = None
        self.symbol = None
        self.order_id_reference = None
        self.size = None
        ...
        self.modify_flags = ModifyFlags(data, current, self)
        if not self.modify_flags.valid:
            self._error = self.modify_flags._error
            return

        current += self.modify_flags._length
...
class AddOrderMessage:
    __slots__ = ('_name', '_error', '_length', '_parent', 'side', 'timestamp', 'symbol', 'order_id', 'size', 'price')

    def __init__(self, data: bytes, offset: int, parent) -> None:
        self._name = "Add Order Message"
        self._error = None
        self._length = 0
        self._parent = parent
        self.side = None
        ...
        self.side = Side(data, current, self)
        if not self.side.valid:
            self._error = self.side._error
            return

        current += self.side.length
        ...
        self.order_id = OrderId(data, current, self)
        if not self.order_id.valid:
            self._error = self.order_id._error
            return

        current += self.order_id.length
...
class RetailLiquidityIndicatorMessage:
    __slots__ = ('_name', '_error', '_length', '_parent', 'retail_liquidity_indicator', 'timestamp', 'symbol')

    def __init__(self, data: bytes, offset: int, parent) -> None:
        self._name = "Retail Liquidity Indicator Message"
        self._error = None
        self._length = 0
        self._parent = parent
        self.retail_liquidity_indicator = None
        self.timestamp = None
        self.symbol = None
        ...
        self.retail_liquidity_indicator = RetailLiquidityIndicator(data, current, self)
        if not self.retail_liquidity_indicator.valid:
            self._error = self.retail_liquidity_indicator._error
            return

        current += self.retail_liquidity_indicator.length
...
def MessageData_factory(data: bytes, offset: int, parent, message_type):
    ...
    match message_type:
        ...
        case "I":
            return RetailLiquidityIndicatorMessage(data, offset, parent)
        ...
        case "a":
            return AddOrderMessage(data, offset, parent)

        case "M":
            return OrderModifyMessage(data, offset, parent)
        ...
        case "T":
            return TradeMessage(data, offset, parent)

        case "B":
            return TradeBreakMessage(data, offset, parent)

        case "C":
            return ClearBookMessage(data, offset, parent)
